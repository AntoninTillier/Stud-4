package bdma.bigdata.project.data.random;

import bdma.bigdata.project.data.Configuration;
import bdma.bigdata.project.data.util.Pool;
import bdma.bigdata.project.data.util.Random;

import static bdma.bigdata.project.data.Configuration.yearStop;
...
public class Student {
    ...
    public static Pool<Student> getPool() {
        if (pool.isEmpty()) {
            for (int year = Configuration.yearStart; year <= yearStop; ++year) {
    ...
    public String getEmailAddress() {
        return firstName + "." + lastName + "@etu.univ-tours.fr";
    }
